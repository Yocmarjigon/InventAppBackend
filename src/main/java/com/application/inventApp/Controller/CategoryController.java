...
public class CategoryController {
  ...
  public ResponseEntity<?> findAll() throws JWTVerificationException {
    List<CategoryDTOFind> categories = categoryService
      .findAll()
      .stream()
      .map(category -> modelMapper.map(category, CategoryDTOFind.class))
      .toList();
  ...
  public ResponseEntity<?> findById(@PathVariable(name = "id") String id) {
    ...
    return ResponseEntity.ok(modelMapper.map(category, CategoryDTOFind.class));
  }

  @PostMapping("/save")
  public ResponseEntity<?> save(
    @Valid @RequestBody CategoryDTOSave categoryDTO,
    BindingResult bindingResult
  ) {
    if (bindingResult.hasFieldErrors()) {
      return new ResponseEntity<>(
        new ResponseOK(bindingResult.getFieldError().getDefaultMessage()),
    ...
    return ResponseEntity.ok(
      new ResponseOK("La categoria fue creada correctamente")
    );
  }
  ...
  public ResponseEntity<?> update(
    ...
    @RequestBody CategoryDTOUpdate categoryDTO
  ) {
    Category category = this.modelMapper.map(categoryDTO, Category.class);
    this.categoryService.update(UUID.fromString(id), category);
    return ResponseEntity.ok(
      new ResponseOK("La categoria fue actualizada correctamente")
    );
  }
  ...
  public ResponseEntity<?> delete(@PathVariable String id) {
    ...
    return ResponseEntity.ok(
      new ResponseOK("La categoria fue eliminada correctamente")
    );
  }
}
